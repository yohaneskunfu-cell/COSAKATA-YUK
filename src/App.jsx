...
// --- KODE TAMBAHAN FITUR GURU ---
// State untuk menyimpan inputan guru
const [teacherInput, setTeacherInput] = useState({ title: '', content: '' });

// Fungsi untuk mengirim Pengumuman baru ke backend/database
const handleCreateAnnouncement = async (e) => {
  e.preventDefault();
  try {
    const response = await fetch('/api/save-data', { // Sesuaikan endpoint API Anda
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        type: 'announcement',
        title: teacherInput.title,
        content: teacherInput.content,
        created_by: 'Guru'
      })
    });
...
<div className="p-6 bg-white rounded-xl shadow-md">
  <h2 className="text-xl font-bold mb-4 text-teal-700">Panel Buat Pengumuman & Tugas (Guru)</h2>
  <form onSubmit={handleCreateAnnouncement} className="space-y-4">
    <div>
      <label className="block text-sm font-medium text-gray-700">Judul Pengumuman</label>
      <input 
        type="text" 
        value={teacherInput.title}
        onChange={(e) => setTeacherInput({ ...teacherInput, title: e.target.value })}
        className="w-full p-2 border rounded-lg" 
        placeholder="Contoh: Jadwal Kuis Kosakata Bab 2" 
        required
      />
    </div>
    <div>
      <label className="block text-sm font-medium text-gray-700">Isi Pesan / Tugas</label>
      <textarea 
        value={teacherInput.content}
        onChange={(e) => setTeacherInput({ ...teacherInput, content: e.target.value })}
        className="w-full p-2 border rounded-lg" 
        placeholder="Tulis instruksi atau soal di sini..." 
        rows="4"
        required
      />
    </div>
